...
export function SimpleMarkdownEditor (): JSX.Element {
  ...
  useEffect(() => {
    ...
    if (localStorageValue === null || localStorageValue === '' || localStorageValue === undefined) {
  ...
  const options = useMemo(() => {
    return {
      ...
      toolbar: [
        ...
        {
          name: 'math-inline',
          action: (editor: any) => { mathButtonFunction(editor, false) },
          className: 'icon-inline-math',
          title: 'math inline'
        },
        {
          name: 'math-block',
          action: (editor: any) => { mathButtonFunction(editor, true) },
          className: 'icon-block-math',
          title: 'math block'
        },
        {
          name: 'page-break',
          action: (editor: any) => { pageBreakButton(editor) },
  ...
  return (
    <>
      ...
      />
      {/* Autosave time */}
      <p className="block w-full text-right text-gray-400 text-xs mr-4">
